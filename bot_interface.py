...
class BotInterface:
    ...
    def search_process(self, user_id):
        ...
        while worksheets:
            ...
            photos_user = self.vk_tools.get_photos(worksheet['id'])
            user_url = f"https://vk.com/id{worksheet['id']}"
            attachments = []
            ...
            self.message_send(user_id,
                              (f'Встречайте {worksheet["name"]}. '
                               f'Ссылка на профиль: {user_url}'),
                              attachment=','.join(attachments))
            save_users(engine, self.params['id'], worksheet['id'])
            break
